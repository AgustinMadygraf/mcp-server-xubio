...
async function runTests() {
  console.log("🚀 Iniciando Suite de Verificación de Herramientas Xubio...\n");
  
  let config;
  try {
    config = validateConfig();
  } catch (e) {
    console.error("❌ Error de configuración. Verifica tu archivo .env");
    process.exit(1);
  }
  ...
  // Registro de todas las herramientas (clonado de index.ts para testeo)
  ...
  // IDs (Solo registro, el test requerirá IDs válidos para ejecución real)
  ...
  console.log(`📊 Herramientas registradas: ${tools.length}/54\n`);
  ...
  for (const tool of tools) {
    ...
    // Saltamos los de ID y PDF por ahora ya que requieren parámetros específicos
    if (tool.name.includes("_por_id") || tool.name.includes("_id") || tool.name === "get_pdf_url" || tool.name === "get_comprobantes_asociados") {
      console.log("⏩ SKIPPED (Requiere parámetros)");
      warningCount++;
      continue;
    }
    ...
    try {
      ...
      // Ejecución de humo (Smoke test)
      await handler?.useCase.execute();
      console.log("✅ OK");
      successCount++;
    } catch (error: any) {
      if (tool.name === "get_pagos") {
        console.log("⚠️ WARNING (Error 500 conocido en Xubio)");
        warningCount++;
      } else {
  ...
  console.log(`⚠️ Advertencias/Saltados: ${warningCount}`);
